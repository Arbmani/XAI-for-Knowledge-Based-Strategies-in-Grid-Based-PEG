...
from action import device, knowledge_based_action_bob_dqn
...
class Experiance_Replay:
    def __init__(self, max_size, start, possible_positions):
        ...
        self.evader_probabilities_memory_t0                 = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device) 
        self.teammate_probabilities_memory_t0               = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device)

        self.teammate_evader_probabilities_memory_t0        = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device) 
        self.teammate_teammate_probabilities_memory_t0      = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device)

        self.agent_position_memory_t0                       = torch.zeros((self.max_size, 2), dtype=torch.float32, device=device) 
        self.evader_probabilities_memory_t1                 = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device) 
        self.teammate_probabilities_memory_t1               = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device)

        self.teammate_evader_probabilities_memory_t1        = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device) 
        self.teammate_teammate_probabilities_memory_t1      = torch.zeros((self.max_size, self.possible_positions), dtype=torch.float32, device=device)


        self.action_memory                                  = torch.zeros(self.max_size, dtype=torch.int64, device=device)    
        self.reward_memory                                  = torch.zeros(self.max_size, dtype=torch.float32, device=device)   
        self.terminal                                       = torch.zeros(self.max_size, dtype=torch.long, device=device)    
        self.time_left1                                      = torch.zeros(self.max_size, dtype=torch.float32, device=device)   
        self.time_left2                                      = torch.zeros(self.max_size, dtype=torch.float32, device=device)  

        self.index          = 0
        self.current_size   = 0

    def add_transition(self, evader_probabilities_t0, teammate_probabilities_t0, teammate_evader_probabilities_t0, teammate_teammate_probabilities_t0, 
                       agent_position_t0, evader_probabilities_t1, teammate_probabilities_t1, teammate_evader_probabilities_t1, teammate_teammate_probabilities_t1, action, reward, terminal, time_left1, time_left2):
        ...
        self.teammate_evader_probabilities_memory_t0[self.index]     =  teammate_evader_probabilities_t0
        self.teammate_teammate_probabilities_memory_t0[self.index]   =  teammate_teammate_probabilities_t0
        ...
        self.teammate_evader_probabilities_memory_t1[self.index]     =  teammate_evader_probabilities_t1
        self.teammate_teammate_probabilities_memory_t1[self.index]   =  teammate_teammate_probabilities_t1
    ...
    def sample(self, number_of_transitions):
        ...
        return (
            self.evader_probabilities_memory_t0[random_transitions],
            self.teammate_probabilities_memory_t0[random_transitions],
            self.teammate_evader_probabilities_memory_t0[random_transitions], 
            self.teammate_teammate_probabilities_memory_t0[random_transitions], 
            self.agent_position_memory_t0[random_transitions],
            self.evader_probabilities_memory_t1[random_transitions], 
            self.teammate_probabilities_memory_t1[random_transitions],
            self.teammate_evader_probabilities_memory_t1[random_transitions],
            self.teammate_teammate_probabilities_memory_t1[random_transitions],
            self.action_memory[random_transitions], 
            self.reward_memory[random_transitions],
            self.terminal[random_transitions],
            self.time_left1[random_transitions],
            self.time_left2[random_transitions]
        )


@dataclass
class state:
    evader_probability  : torch.Tensor
    teammate_probability: torch.Tensor
    teammate_evader_probability  : torch.Tensor
    teammate_teammate_probability: torch.Tensor
...
def loss(dqn, target_dqn, transitions, gamma):
    (evader_probabilities_t0s,
     ...
     teammate_evader_probabilities_t0s,
     teammate_teammate_probabilities_t0s,
     agent_position_t0s,
     evader_probabilities_t1s,
     teammate_probabilities_t1s,
     teammate_evader_probabilities_t1s,
     teammate_teammate_probabilities_t1s,
     actions,
     rewards,
     terminal,
     time_left1,
    ...
    q_values_t0         = dqn(evader_probabilities_t0s, teammate_probabilities_t0s, teammate_evader_probabilities_t0s, teammate_teammate_probabilities_t0s, agent_position_t0s,time_left1)
    ...
    with torch.no_grad():
        if nonterminal.any().item():
            ...
            q_values_t1 = dqn(evader_probabilities_t1s[nonterminal], teammate_probabilities_t1s[nonterminal], teammate_evader_probabilities_t1s[nonterminal], teammate_teammate_probabilities_t1s[nonterminal], agent_position_t1s, time_left2[nonterminal])
            q_values_target_t1 = target_dqn(evader_probabilities_t1s[nonterminal], teammate_probabilities_t1s[nonterminal], teammate_evader_probabilities_t1s[nonterminal], teammate_teammate_probabilities_t1s[nonterminal], agent_position_t1s, time_left2[nonterminal])
...
def train():
    ...
    def add_transition_helper(memory, state0, state1):
        ...
        memory.add_transition(
                        ...
                        teammate_evader_probabilities_t0    = state0.teammate_evader_probability, 
                        teammate_teammate_probabilities_t0  = state0.teammate_teammate_probability, 
                        ...
                        teammate_evader_probabilities_t1    = state1.teammate_evader_probability, 
                        teammate_teammate_probabilities_t1  = state1.teammate_teammate_probability,
    ...
    while(completed_simulations < simulations):
        ...
        for index, running_index in enumerate(running_indexes):
            (p1_agent_position[running_index], 
             ...
             p1_teammate_evader_probability[running_index], 
            ...
            p1_states1 = state(
                                ...
                                teammate_evader_probability = p1_teammate_evader_probability[running_index],
        ...
        for index, running_index in enumerate(p2_running_indexes):
            (p2_agent_position[running_index], 
             ...
             p2_teammate_evader_probability[running_index], 
             p2_teammate_teammate_probability[running_index], 
             )  = p2_results[index]
            ...
            p2_states1 = state(
                    ...
                    teammate_evader_probability = p2_teammate_evader_probability[running_index],
