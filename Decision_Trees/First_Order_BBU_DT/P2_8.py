...
def interpretable_strategy(features):
    if features["Evader(DOWN) - Evader(LEFT)"] <= 0.000007:
        if features["Evader(LEFT) - Agent_Row"] <= -3.632217:
            return 0 # WALK LEFT
        else:
            if features["Evader(LEFT) - Teammate(RIGHT)"] <= 0.124686:
                return 3 # WALK UP
            else:
                if features["Evader(DOWN) - Teammate(RIGHT)"] <= 0.236416:
                    return 2 # WALK DOWN
                else:
                    return 1 # WALK RIGHT
    else:
        if features["Agent_Row - Agent_Column"] <= -6.500000:
            ...
            if features["Evader(LEFT) - Evader(RIGHT)"] <= -0.329961:
