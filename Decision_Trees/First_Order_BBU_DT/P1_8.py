...
def interpretable_strategy(features):
    if features["Teammate(LEFT) - Time_Left"] <= -0.679556:
        if features["Evader(UP) - Agent_Row"] <= -3.720640:
            return 1 # WALK RIGHT
        else:
            return 2 # WALK DOWN
    else:
        if features["Evader(UP) - Evader(RIGHT)"] <= -0.000006:
            if features["Agent_Row - Agent_Column"] <= -1.500000:
                return 1 # WALK RIGHT
            else:
                if features["Evader(UP) - Evader(DOWN)"] <= -0.315571:
                    return 3 # WALK UP
                else:
                    return 1 # WALK RIGHT
        else:
            if features["Evader(DOWN) - Evader(LEFT)"] <= -0.137978:
