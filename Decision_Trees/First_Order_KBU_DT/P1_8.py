...
def interpretable_strategy(features):
    if features["Evader(DOWN) - Evader(LEFT)"] <= 0.123533:
        if features["Evader(UP) - Evader(RIGHT)"] <= 0.069938:
            if features["Evader(LEFT) - Time_Left"] <= -0.028283:
                if features["Teammate(UP) - Agent_Column"] <= -2.722212:
                    return 2 # WALK DOWN
                else:
                    return 0 # WALK LEFT
            else:
            ...
            if features["Evader(LEFT) - Agent_Column"] <= -5.805938:
                return 2 # WALK DOWN
            else:
