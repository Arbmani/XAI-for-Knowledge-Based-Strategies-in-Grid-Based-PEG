...
def interpretable_strategy(features):
    if features["E(LEFT) - E(RIGHT)"] <= -0.343442:
        if features["T(LEFT) - agent_row"] <= -9.999841:
            return 0
        else:
            if features["T(up) - T(RIGHT)"] <= 0.018590:
                if features["E(RIGHT) - T(RIGHT)"] <= -0.230856:
                    return 0
                else:
                    return 3
            else:
        ...
        if features["E(RIGHT)"] <= 0.000066:
            if features["T(RIGHT) - agent_column"] <= -7.999975:
                return 1
            else:
                return 2
        else:
            if features["E(DOWN) - E(LEFT)"] <= 0.179352:
                if features["agent_column"] <= 12.500000:
                    if features["E(RIGHT) - T(RIGHT)"] <= 0.341710:
                        if features["E(UP) - T(DOWN)"] <= 0.737715:
                            return 0
                        else:
                            return 2
                    else:
                ...
                if features["E(UP) - T(RIGHT)"] <= 0.001825:
                    return 1
                else:
                    if features["E(LEFT) - E(RIGHT)"] <= 0.006805:
                        if features["E(DOWN) - T(up)"] <= 0.457334:
                            return 3
                        else:
                            return 2
                    else:
